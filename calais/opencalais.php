...
class OpenCalais {
    ...
    public function getEntities($document) {
		...
		if (!empty($item->_typeReference)) {
			$ref = substr($item->_typeReference,strrpos($item->_typeReference,"/")+1,strlen($item->_typeReference));
	                $this->entities[$item->_typeGroup][$ref][] = trim($item->name);
		} else {
